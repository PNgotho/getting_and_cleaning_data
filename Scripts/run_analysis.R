...
setwd("D:\\R_Training/week4project/Dataset")
...
x_test <- read.fwf("./test/X_test.txt", widths = thewidths, header = FALSE)

subject_train <- read.delim("./train/subject_train.txt", header = FALSE)
y_train <- read.delim("./train/y_train.txt", header = FALSE)
x_train <- read.fwf("./train/X_train.txt", widths = thewidths, header = FALSE)
